...
about.add_command(label='Text Editor Version', compound= tk.LEFT, accelerator="(1.0)")
...
#selected font goes inbelow variable
...
font_size.current(4)  #for 12 value
...
#wrap according to words
text_editor.config(wrap='word', relief= tk.FLAT)

schroll_bar = tk.Scrollbar(main_application)
text_editor.focus_set()
schroll_bar.pack(side=tk.RIGHT, fill=tk.Y)
text_editor.pack(fill=tk.BOTH, expand=True)
schroll_bar.config(command=text_editor.yview)
text_editor.config(yscrollcommand=schroll_bar.set)
...
#function to change font size   #global is bcz we are changing values of global varialble
...
def change_bold():
    #gives dictionary that contain weight
...
# align left
def align_left():
    #every thing writen in text editior store in this variable
    text_content = text_editor.get(1.0, 'end')
    text_editor.tag_config('left', justify=tk.LEFT)
    #delete text
    text_editor.delete(1.0, tk.END)
    #insert test in position
...
#  align right
def align_right():
    #every thing writen in text editior store in this veriable
    text_content = text_editor.get(1.0, 'end')
